...
SIDAI::SIDAI(int x, PAI *dai1, PAI *dai2) : x(x), dai1(dai1), dai2(dai2), PAI(PAI_TYPE::SIDAI_TYPE) {}
ostream &SIDAI::output() {
    cout << "SI : " << name[x] << " DAI : ";
    if (dai1->type == PAI_TYPE::DAN_TYPE) {
        dai1->output();
        dai2->output();
    } else if (dai1->type == PAI_TYPE::DUIZI_TYPE) {
        dai1->output();
        if (dai2->type == PAI_TYPE::DUIZI_TYPE) {
            dai2->output();
        } 
    }
    return cout;
...
bool SIDAI::operator>(PAI *pre) {
    switch (pre->type) {
        ...
        case PAI_TYPE::SIDAI_TYPE: {
            ...
            if (this->dai1->type != pre_->dai1->type && this->dai2->type != pre_->dai2->type) return false;
            return this->x > pre_->x;
        } break;
        default: return false;
    }
...
vector<PAI *> SIDAI::get(int *arr) {
    ...
    for (int i = 3; i < MAX_N; ++i) {
        ...
        for (int j = 3; j < MAX_N; ++j) {
            ...
            if (arr[j] < 2 || i == j)
                continue;
            ret.push_back(new SIDAI(i, new DUIZI(j), new PASS()));
            for (int k = j + 1; k < MAX_N; ++k) {
                if (arr[k] < 2 || i == j) 
                    continue;
...
vector<PAI *> PAI::getPai(int *arr) {
    ...
    vector<PAI *> temp6 = SIDAI::get(arr);
    vector<PAI *> temp7 = ZHADAN::get(arr);
    vector<PAI *> temp8 = WANGZHA::get(arr);
